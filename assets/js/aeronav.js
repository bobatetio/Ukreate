/* Aerodynamics-style navbar behaviour: mobile menu toggle + scroll-aware pill.
   Idempotent (safe across swup page transitions); no dependency on the Izanami bundle. */
(function () {
  function init() {
    ...
    if (burger && links) {
      var closeBtn = links.querySelector('.aeroNav_mobileClose');
      var setOpen = function (open) {
        burger.classList.toggle('is-open', open);
        links.classList.toggle('is-open', open);
        // the nav is its own stacking context, so it has to outrank page-level
        // fixed furniture (e.g. the /for-creators/ banner) while the panel is up
        nav.classList.toggle('is-menuOpen', open);
        burger.setAttribute('aria-expanded', open ? 'true' : 'false');
        // the panel scrolls on its own; hold the page still behind it
