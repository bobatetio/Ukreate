...
(function () {
  ...
  var PER_CARD = 330;                // px of scroll per card on the ride
  ...
  function init() {
    ...
    var philo = null;
    var stage = document.querySelector('.ukFeatMoved');
    function pickTarget() {
      // land on the open first card of the creators row
      philo = document.querySelector('.ukEx_item--dock') || document.querySelector('.ukEx_item[data-i="0"]') || document.querySelector('.homePhilosophy_singleImg');
      if (!philo) {
    ...
    function frame(now) {
      ...
      if (vw <= 900) {
        ...
        // pulled left of the right gutter so the rail has room for ~6 cards, and
        // sat just below centre so it clears the copy block and reads diagonally
        sl = vw - g - sw;
        st = (vhh - sh) / 2 + 96;   // formation seated lower in the hero
      }
      var ox = sl + sw * 0.40;
      ...
      var targetVid = philo ? philo.querySelector('video') : null;
      var targetRadius = philo ? (parseFloat(getComputedStyle(philo).borderTopLeftRadius) || 14) : 14;
      ...
      for (var i = 0; i < cards.length; i++) {
        ...
        var isDocked = i === 0 && docked;

        // video playback: autoplay muted for every card while it's in the
        // hero, and for the docked first video — independent of hover.
        // Hover only drives the turn-to-face + the profile-chip reveal below.
        var v = vids[i];
        if (v) {
          var want = interactive || isDocked;
        ...
        // followers hide inside 01 once seated
        if (i > 0 && docked) op = 0;
        // Card 01 flies down, grows into the reserved slot's exact box and corner
        // radius, and STAYS there as the row's first card. The other cards sit
        // folded to its right. Nothing is swapped or removed.
        var curW = CW, curH = CH;
        if (i === 0 && e < 0 && pr) {
          var mu = smooth(clamp((clamp(-e / E, 0, 1) - 0.55) / 0.45, 0, 1));
          curW = CW + (pr.width - CW) * mu;
          curH = CH + (pr.height - CH) * mu;
          card.style.width = curW.toFixed(1) + 'px';
          card.style.height = curH.toFixed(1) + 'px';
          card.style.borderRadius = (14 + (targetRadius - 14) * mu).toFixed(1) + 'px';
          // match the frame, then step aside; the row's own card is identical
          // and takes over, so the first card behaves like every other one
          if (mu > 0.7 && targetVid && vids[0]) {
            if (Math.abs(targetVid.currentTime - vids[0].currentTime) > 0.12) {
              try { targetVid.currentTime = vids[0].currentTime; } catch (err) {}
            }
            if (targetVid.paused) { var tp = targetVid.play(); if (tp && tp.catch) tp.catch(function () {}); }
          }
          op = mu >= 0.985 ? 0 : 1;
          if (mu >= 0.985 && stage && !stage.classList.contains('is-landed')) {
            stage.classList.add('is-landed');
            document.dispatchEvent(new CustomEvent('ukex:reset'));
          }
          // leaving the dock re-arms the sequence for the next pass
          if (mu < 0.5 && stage && stage.classList.contains('is-landed')) {
            stage.classList.remove('is-landed');
            document.dispatchEvent(new CustomEvent('ukex:reset'));
          }
        } else if (i === 0) {
          card.style.width = ''; card.style.height = ''; card.style.borderRadius = '';
        }

        card.style.transform = 'translate3d(' + (x - curW / 2) + 'px,' + (y - curH / 2) + 'px,' + z + 'px)' +
        ...
        // card 01 stays hoverable after it lands, so its chip behaves like the rest
        var hoverable = interactive;   // handed-over card must not swallow hover
        card.style.pointerEvents = hoverable && op >= 0.5 ? 'auto' : 'none';
